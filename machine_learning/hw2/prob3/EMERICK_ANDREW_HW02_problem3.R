...
plot_linear_svm <- function(costs, errors, outfile){
    ...
    graphics.off()
    pdf(outfile)
    plot(costs,errors, xlab='Margin Parameter (cost)', ylab ='Misclassification Rate',xlim=c(2**(-10),1),ylim=c(0,0.25),cex=2)
    min_err <- min(errors)
    ...
    legend(x='topleft',c('All points', sprintf('Best C = %f',min_costs[1])),col=c('black','red'),pch=c(1,1),cex=1.5)
    dev.off()
    graphics.off()
} # end plot linear SVM
...
plot_non_linear_svm <- function(parameters, gammas, errors, best_param, outfile){
    ...
    legend(x='topright',c("log(Gamma) = -4","log(Gamma) = -3", "log(Gamma) = -2", "log(Gamma) = -1",sprintf("Best (C,gamma) = (%f,%f)",best_param[1], best_param[1,2])),
